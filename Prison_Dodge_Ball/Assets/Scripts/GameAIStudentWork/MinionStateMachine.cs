...
namespace GameAIStudent
{
    ...
    public class MinionStateMachine : MonoBehaviour
    {
        ...
        class GoToThrowSpotState : MinionState
        {
            ...
            private int CalculateActiveMinions(MinionScript[] minions)
            {
                int count = 0;
            ...
            private int CalculateOpponentActiveMinions(PrisonDodgeballManager.OpponentInfo[] opponents)
            {
                int count = 0;
                if (opponents != null)
                {
            ...
            {
                base.Enter();
                Minion.GoTo(Mgr.TeamCenter(Team).position + 0.5f * new Vector3(Random.value, 0f, 0f));
            ...
            public override StateTransitionBase<MinionFSMData> Update()
            {
                ...
                {
                    int ourActiveMinions = CalculateActiveMinions(TeamData.TeamMates);
                    // Use the GetAllOpponentInfo method to get active opponents' information.
                    PrisonDodgeballManager.OpponentInfo[] opponentsInfo = null;
                    Mgr.GetAllOpponentInfo(Team, ref opponentsInfo);
                    int opponentActiveMinions = CalculateOpponentActiveMinions(opponentsInfo);

                    if (FindRescuableTeammate(out var m))
                    {
                        //Debug.Log("Velocity: " + m.Velocity.magnitude);
                        if (ourActiveMinions < opponentActiveMinions)
                        {   
                            // Ensure the teammate is not moving (e.g., has just been tagged and is on their way to prison).
                            if (m.Velocity.magnitude <= 0.05f)
                            {   
                ...
                return null; // No state transition by default.
            }
        ...
        class RescueState : MinionState<MinionScript>
        {
            ...
            const float attemptDuration = 5.0f; // Max time to attempt rescue before transitioning out.
            bool hasMoved = false;
            ...
            public override StateTransitionBase<MinionFSMData> Update()
            {
                ...
                else if (Minion.HasBall && (Time.timeSinceLevelLoad - stateEnterTime > attemptDuration))
                {
                    return ParentFSM.CreateStateTransition(ThrowBallStateName);
                }
        ...
        class ThrowBallState : MinionState
        {
            ...
            public override StateTransitionBase<MinionFSMData> Update()
            {
                ...
                if (!hasOpponent)
                    if (FindRescuableTeammate(out var m1)) {
                        if (m1.Velocity.magnitude <= 0.05f)
                        {
                            return ParentFSM.CreateStateTransition<MinionScript>(RescueStateName, m1, true);
                        }
                    }
                ...
                if (selection == ShotSelection.SelectThrowReturn.DoThrow)
                {
                    ...
                    else
                    {
                        ...
                        if (FindRescuableTeammate(out var m2))
                        {
                            if (m2.Velocity.magnitude <= 0.05f)
                            {
                                return ParentFSM.CreateStateTransition<MinionScript>(RescueStateName, m2, true);
                            }
                        }
                    }
                }
        ...
        class DefensiveDemoState : MinionState
        {
            ...
            {
                var r = Minion.GoTo(Mgr.TeamHome(Team).position + 7f * (new Vector3(Random.value, 0f, Random.value)));

                if (!r)
                {
                    Minion.GoTo(Mgr.TeamHome(Team).position + 6f * (new Vector3(Random.value, 0f, Random.value)));
            ...
            public override void Enter()
            {
                ...
                evadeWaitTimeSec = 0.5f * Minion.EvadeCoolDownTimeSec + 0.1f;
            }
            ...
            public override StateTransitionBase<MinionFSMData> Update()
            {
                ...
                else if (Time.timeSinceLevelLoad - lastEvade >= evadeWaitTimeSec)
                {
                    ...
                    Minion.Evade(ev, Random.Range(0.9f, 1.0f));
                }
